/**
 * Moonbeam Card Component
 * 
 * Displays GLMR balance and information from Moonbeam Network.
 */
...
import { queryMoonbeamBalance, getMoonbeamConfig } from '@/lib/chains/moonbeam';
import { formatBalance, formatUSD } from '@/lib/utils/formatters';
import { ExternalLink, RefreshCw } from 'lucide-react';

export function MoonbeamCard() {
  const { selectedAccount, isConnected } = useWallet();
  const { balance, isLoading, error, refetch } = useChainBalance(
    queryMoonbeamBalance,
    selectedAccount?.address || null,
    { enabled: isConnected }
  );

  const config = getMoonbeamConfig();
      ...
      <CardContent>
        ...
        <div className="mt-4 flex items-center justify-between">
          <a
            ...
            className="flex items-center gap-2 text-sm text-[var(--text-secondary)] hover:text-[var(--moonbeam-teal)] transition-colors"
          >
